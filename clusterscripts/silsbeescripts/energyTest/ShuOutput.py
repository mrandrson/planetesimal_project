...
#    sim.ri_ias15.min_dt = 10.0**6
    ...
    Evec = np.zeros(N)
    for i in range(0, N):
        Evec[i] = getEnergy(sim.particles[i], tvec[Nsteps])/(0.5*v_0**2)
    return Evec

Evec = get_captured_number(bmin, bmax, v_0, t_final)
    
np.savetxt('./data/ShuEnergies/'  +  'v' + v_index + 'b' + b_index+ '.txt', Evec )
