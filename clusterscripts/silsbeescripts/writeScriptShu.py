...
def writeScript(bmin, bmax, v_0, t_final, bindex, vindex, folder, reps):
    ...
    lines = lines + ['python ShuOutput.py ' + str(bmin) + ' ' + str(bmax) + ' '  + str(v_0) + ' ' + str(t_final) +  ' '  + str(bindex) + ' ' + str(vindex) + ' ' + str(folder) + ' ' + str(reps) + '\n']
    arc = open('submit.sh', 'w')
    arc.writelines(lines)
    arc.close()

cs = math.sqrt(3.36)*100
bvec = np.logspace(9, 18, 10)
Nv = 10
vvec = np.logspace(np.log10(cs/10), np.log10(cs*5), Nv)
#vvec = np.logspace(-3, -1, Nv)

tot = Nv*(len(bvec)-1)
prog=0
for i in range(0, len(bvec)-1):
    for j in range(0, len(vvec)):
         writeScript(bvec[i], bvec[i+1],  vvec[j], 1.0829*10.0**13, i, j, 1, 100)
         os.system('source submit.sh')
         prog += 1
         print(f"{(prog/tot)*100:.2f}%")
...
cs = math.sqrt(3.36) * 100
#bvec = np.logspace(9, 17, 80)
bvec = np.logspace(9, np.log10(3e18), 80)
Nv = 30
vvec = np.logspace(np.log10(cs / 10), np.log10(cs * 5), Nv)
