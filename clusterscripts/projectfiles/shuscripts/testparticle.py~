...
def partial_phi_partial_t(r, t):
    h = 1e-4
    dt = years_to_seconds(h)
    phi_plus = getPhi(r, t + dt)
    phi_minus = getPhi(r, t - dt)
...
def Edot(particle, t):
    h = 1e-4
    dt = years_to_seconds(h)
    return (getEnergy(particle, t + dt) - getEnergy(particle, t - dt)) / (2 * dt)
...
def simulateparticle(v0, t_f): 
    ...
    for i in range(0, Nsteps):
        ...
        for k, particle in enumerate(sim.particles):
            ...
            dphi_dt = partial_phi_partial_t(r, sim.t)
            dE_dt = Edot(particle, sim.t)
            ...
            particle_dat[f'{k}'][f'{tyr}'] = {
                'energy': energy,
                'r': r,
                'potential': potential,
                'impact parameter': impact_param,
                'velocity': v,
                'dphi_dt': dphi_dt,
                'dE_dt': dE_dt
            }
    ...
    structured_array = np.array(rows, dtype=dtype)

    with h5py.File("./outputdata/simdata.h5", "a") as f:
        group_name = f"v0_{v0:.2f}"
        if group_name in f:
            del f[group_name]
        f.create_dataset(group_name, data=structured_array)
        f[group_name].attrs["v0"] = v0
