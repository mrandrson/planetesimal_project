...
t_init = -float(sys.argv[3])*24*3600*365.25
t_entry = -float(sys.argv[4])*24*3600*365.25
folder = sys.argv[5]
...
def get_captured_number(b, n_0, v_0, t_init, t_entry): #t_init in seconds
...
#    sim.ri_ias15.min_dt = 10.0**6
    ...
    for i in range(0, 100):
        print(N, i)
        if t_entry < sim.t - wait_time + t_init and particleAdded == 0:
            R_max = get_r_max(t_init)
            [vxp, vyp] = get_params(get_phi_Larson, b, R_max, v_0, t_init)
    ...
    tvec = -np.flip(np.logspace(np.log10(-t_init/10.0**8), np.log10(-t_init), 1000))
...
for i in range(0, 1):
    s = s + get_captured_number(b, 10**(-45), v_0, t_init, t_entry)
