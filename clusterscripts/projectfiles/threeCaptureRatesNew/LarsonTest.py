    ...
    for i in range(0, 10):
        ...
        for j in range(0, N_static):
            ...
            if b > bmiss:
                ...
                N = N + 1
        if N > 0:
            sim.integrate((i+1)*wait_time/10)
        else: 
            sim.t = (i+1)*wait_time/10
    tvec = -np.flip(np.logspace(np.log10(-t_init/10.0**8), np.log10(-t_init), 10))
    for i in range(0, 9):
        print(N, i)
        R_max = get_r_max(tvec[i])
        bmiss = get_b_max(get_phi_Larson, R_max, v_0*1.025, tvec[i])
        if bmiss > math.sqrt(bmin*bmax):
            N_exp = -1/t_init*(tvec[i+1] - tvec[i])*get_reduction_factor_Larson(math.sqrt(bmin*bmax), R_max, v_0, tvec[i])
        else:
            N_exp = 0
        N_static = np.random.poisson(N_exp)
        for j in range(0, N_static):
