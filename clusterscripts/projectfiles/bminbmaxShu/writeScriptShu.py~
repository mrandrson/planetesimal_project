    ...
    with open(job_list_name, 'w') as job_list:
        for i in range(len(bvec) - 1):
            for j, v_0 in enumerate(vvec):
                job_list.write(f'python Shu.py {bvec[i]} {bvec[i+1]} {v_0} {t_final} {i} {j} {folder} {reps}\n')
...
#1e12 for Rfin = Rsun/10

script = writeMasterScript(bvec, vvec, 1.0829 * 10.0**12, 1, 100, max_jobs=80)
