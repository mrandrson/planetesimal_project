...
bvec = np.logspace(9, 17, 40)
Nv = 20
vvec = np.logspace(np.log10(cs/10), np.log10(cs*5), Nv)
for i in range(0, 39):
    for j in range(0, Nv):
         writeScript(bvec[i], bvec[i+1],  vvec[j], 1.0829*10.0**11, i, j, 1, 30)
...
for i in range(0, 39):
    for j in range(0, Nv):
         writeScript(bvec[i], bvec[i+1],  vvec[j], 1.0829*10**12, i, j, 2, 30)
         os.system('sbatch submit.sh')
...
for i in range(0, 39):
    for j in range(0, Nv):
         writeScript(bvec[i], bvec[i+1],  vvec[j], 1.0829*10.0**13, i, j, 3, 30)
         os.system('sbatch submit.sh')
