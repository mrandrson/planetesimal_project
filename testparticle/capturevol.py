...
earr = np.loadtxt('energies.txt')
energy = earr[:,1]
v0 = earr[:,0]
for v in np.unique(v0):
    energies_at_v = energy[v0 == v]
    count_captured = np.sum(energies_at_v < 0)
    Ncapt[v] = count_captured
...
def plotmean(x, y, nbins, label, vlines=False):
    ...
    plt.plot(xmid, ymean, label = f'{label}')
...
# np.savetxt('capturevolRsun_100.txt', captVolvec)


Rsun = np.loadtxt('capturevolRsun.txt')
plotmean(Rsun[:,0]/c_s, Rsun[:,1],30, r'$\left(R_0\right)_f=R_\odot$')
  
Rsun_10 = np.loadtxt('capturevolRsun_10.txt')
plotmean(Rsun_10[:,0]/c_s, Rsun_10[:,1],30, r'$\left(R_0\right)_f=R_\odot/10$')
  
plotmean(captVolvec[:, 0]/c_s, captVolvec[:, 1],25, r'$\left(R_0\right)_f=R_\odot/100$')

    
'''  
Rsun = np.loadtxt('capturevolRsun.txt')
plotmean(Rsun[:,0]/c_s, Rsun[:,1]/np.max(Rsun[:,1]),30, r'$\left(R_0\right)_f=R_\odot$')
  
Rsun_10 = np.loadtxt('capturevolRsun_10.txt')
plotmean(Rsun_10[:,0]/c_s, Rsun_10[:,1]/np.max(Rsun_10[:,1]),30, r'$\left(R_0\right)_f=R_\odot/10$')
  
plotmean(captVolvec[:, 0]/c_s, captVolvec[:, 1]/np.max(captVolvec[:,1]),30, r'$\left(R_0\right)_f=R_\odot/100$')
''' 
 
plt.yscale('log')
plt.xscale('log')
#plt.ylim(1e7, 1.1*max(Rsun[:, 1]))
plt.xlim(0, 2)
plt.legend()
...
#plt.savefig('capture_volume_plot.svg', format='svg', bbox_inches='tight')
